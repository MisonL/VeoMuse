...
export class OrganizationService {
  static ensureDefaultOrganization() {
    const db = getLocalDb()
    const existing = db.prepare(`SELECT * FROM organizations WHERE id = 'org_default' LIMIT 1`).get()
  ...
  static requireRole(organizationId: string, userId: string, role: OrganizationRole): OrganizationRole {
  ...
  static addMemberByEmail(organizationId: string, email: string, role: OrganizationRole): OrganizationMember {
    const normalizedEmail = (email || '').trim().toLowerCase()
    const db = getLocalDb()
    const userRow = db.prepare(`SELECT id, email FROM users WHERE email = ? LIMIT 1`).get(normalizedEmail) as { id: string; email: string } | null
