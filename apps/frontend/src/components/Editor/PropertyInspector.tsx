...
import TelemetryDashboard from './TelemetryDashboard'; // 引入遥测组件
...
const PropertyInspector: React.FC = () => {
  ...
  const handleTranslateAndVoice = async (targetLang: string) => {
    if (!selectedClip?.data?.content || targetLang === 'none') return;
    setIsProcessing(true);
    showToast(`🧬 正在执行语言炼金: ${targetLang}`, 'info');
    try {
      const { data: trans, error: transError } = await api.api.ai.translate.post({ text: selectedClip.data.content, targetLang });
      if (transError) {
  ...
  return (
    <div className="inspector-panel glass-panel">
      <header className="inspector-header">
        ...
        {selectedClip && <span className="clip-type-badge">{selectedClip.type}</span>}
      </header>
