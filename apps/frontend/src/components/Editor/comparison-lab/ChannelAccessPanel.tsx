      ...
      <div key={row.id} className="capability-row">
        ...
        >
          ...
          {row.id === 'openai-compatible' ? (
            <>
              ...
              <input
                ...
                onChange={(event) => onUpdateChannelForm(row.id, { temperature: event.target.value })}
          ...
          <div className="lab-inline-actions">
            <button type="button" onClick={() => onTestChannelConfig(row.id)}>测试</button>
    ...
    <div className="channel-panel-mask" role="dialog" aria-modal="true" aria-label="AI 渠道接入状态" data-testid="area-channel-panel-mask">
      <section className="channel-panel" data-testid="area-channel-panel">
        <header className="channel-panel-head">
          ...
          <button type="button" className="channel-close-btn" onClick={onClose} data-testid="btn-close-channel-panel">关闭</button>
        </header>

        <div className="channel-panel-actions">
          <button type="button" className="channel-refresh-btn" onClick={onLoadCapabilities} disabled={isCapabilitiesLoading}>
            {isCapabilitiesLoading ? '刷新中...' : '刷新状态'}
          </button>
          <button type="button" className="channel-refresh-btn" onClick={onRefreshChannelConfigs} disabled={!effectiveOrganizationId}>
            刷新配置
          </button>
              ...
              <div className="channel-list">
                <div className="capability-row">
                  <div className="channel-row-controls">
                    ...
                    />
                    <div className="lab-inline-actions">
                      <button onClick={onCreateOrganization} data-testid="btn-create-organization">创建组织</button>
                      <button onClick={onLogoutAuth} data-testid="btn-logout-auth">退出登录</button>
                    </div>
                    <input
                    ...
                    <select
                      ...
                      value={inviteOrgRole}
                      onChange={(event) => onInviteOrgRoleChange(event.target.value as OrganizationRole)}
                    >
                ...
                <div className="capability-row">
                  <div className="channel-row-controls">
                    <select
                      ...
                      onChange={(event) => onActiveChannelScopeChange(event.target.value as 'organization' | 'workspace')}
                    >
                ...
                <div className="capability-row">
                  <div className="channel-row-controls">
                    ...
                    <input
                      ...
                      onChange={(event) => onQuotaFormChange({ storageLimitMb: event.target.value })}
                      placeholder="存储配额 MB（0 为不限制）"
                    />
                    <input
                      name="quotaConcurrencyLimit"
                      value={quotaForm.concurrencyLimit}
                      onChange={(event) => onQuotaFormChange({ concurrencyLimit: event.target.value })}
                      placeholder="并发配额（0 为不限制）"
                    />
                    <div className="lab-inline-actions">
                      <button type="button" onClick={onSaveOrganizationQuota} data-testid="btn-save-org-quota">保存配额</button>
                      <button type="button" onClick={onRefreshOrganizationQuota} data-testid="btn-refresh-org-quota">刷新配额</button>
                    </div>
                    <div className="lab-inline-actions">
                      <button type="button" onClick={() => onExportOrganizationAudits('json')} data-testid="btn-export-org-audit-json">导出审计 JSON</button>
                      <button type="button" onClick={() => onExportOrganizationAudits('csv')} data-testid="btn-export-org-audit-csv">导出审计 CSV</button>
                    </div>
                    <div className="capability-meta">
                      <span>
