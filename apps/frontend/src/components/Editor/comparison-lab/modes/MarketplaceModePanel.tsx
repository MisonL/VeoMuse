      ...
      <div className="marketplace-policy">
        ...
        <label className="lab-field">
          ...
          <select
            ...
            onChange={(event) => onSelectedPolicyChange(event.target.value)}
          >
        ...
        <div className="policy-create-grid">
          ...
          <label className="lab-field">
            ...
            <select
              ...
              value={policyCreatePriority}
              onChange={(event) =>
                onPolicyCreatePriorityChange(event.target.value as PolicyPriority)
              }
            >
        ...
        <div className="policy-controls">
          <label>
            ...
            <input
              ...
              step={0.1}
              value={policyBudget}
              onChange={(e) => onPolicyBudgetChange(Number(e.target.value || 0))}
            />
          </label>
          <label>
            优先级
            <select
              ...
              onChange={(e) => onPolicyPriorityChange(e.target.value as PolicyPriority)}
            >
              <option value="quality">质量</option>
              <option value="speed">速度</option>
              <option value="cost">成本</option>
        ...
        {policyDecision ? (
          <div className="policy-result">
            <div>
              推荐模型：<b>{policyDecision.recommendedModelId}</b>
            </div>
            ...
            {Array.isArray(policyDecision.scoreBreakdown) &&
            policyDecision.scoreBreakdown.length > 0 ? (
              <div className="policy-breakdown">
                {policyDecision.scoreBreakdown.map((item) => (
                  <div key={item.modelId}>
                    {item.modelId}: Q{item.quality.toFixed(2)} / S{item.speed.toFixed(2)} / C
                    {item.cost.toFixed(2)} / R{item.reliability.toFixed(2)} {'=>'}{' '}
                    <b>{item.finalScore.toFixed(2)}</b>
                  </div>
                ))}
              </div>
            ) : null}
      ...
      <div className="market-right-stack">
        ...
        <div className="execution-panel">
          <div className="execution-head">
            <h4>策略执行记录</h4>
            <button
              disabled={!selectedPolicyId || policyExecLoading}
              onClick={() => onLoadPolicyExecutions(true)}
            >
              刷新
            </button>
          </div>
          <div className="execution-list">
            ...
            {policyExecutions.length === 0 ? (
              <div className="api-empty">暂无策略执行记录</div>
            ) : null}
          </div>
          {policyExecHasMore ? (
            <button
