...
const OpsToolsSection: React.FC<OpsToolsSectionProps> = ({
  ...
  const alertsTone =
    openReliabilityAlerts.length > 0
      ? criticalReliabilityAlerts.length > 0
        ? 'critical'
        : 'warning'
      : 'neutral'
  const ackTone = acknowledgedReliabilityAlerts.length > 0 ? 'success' : 'neutral'
  const budgetTone = resolveOpsTone(errorBudget?.evaluation.status)
  const rollbackTone = resolveOpsTone(rollbackDrillResult?.status)
  const watchTone =
    criticalReliabilityAlerts.length > 0
      ? 'critical'
  ...
  return (
    <section className="collab-card">
      ...
      <div className="collab-workflow-cluster collab-workflow-cluster--budget">
        ...
        <div className={`ops-budget-card ops-budget-card--${budgetTone}`}>
          ...
          {rollbackDrillResult ? (
            <div className="collab-list-item collab-list-item--rich">
              <div className="collab-list-item-head">
                ...
                <span className={`lab-status-badge lab-status-badge--${rollbackTone}`}>
