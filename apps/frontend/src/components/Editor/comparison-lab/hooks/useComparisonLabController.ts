...
export const useComparisonLabController = ({
  ...
  const [workspaceName, setWorkspaceName] = useState('VeoMuse 项目空间')
  const [workspaceOwner, setWorkspaceOwner] = useState('空间管理员')
  const [workspaceId, setWorkspaceId] = useState('')
  const [projectId, setProjectId] = useState('')
  const [memberName, setMemberName] = useState('协作成员 A')
  const [collabRole, setCollabRole] = useState<WorkspaceRole>('editor')
  const [inviteRole, setInviteRole] = useState<WorkspaceRole>('editor')
  const [inviteCode, setInviteCode] = useState('')
  const [uploadFileName, setUploadFileName] = useState('rough-cut-v1.mp4')
