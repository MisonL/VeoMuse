...
export const useV4CreativeOps = ({
  ...
  const showV4CreativeError = useCallback(
  ...
  const runV4CreativeTask = useCallback(
    async <T>(
      task: () => Promise<T>,
      fallbackMessage: string,
      options?: { guardBusy?: boolean }
    ) => {
      if (options?.guardBusy !== false && isV4CreativeBusy) return null
      setIsV4CreativeBusy(true)
      try {
        return await task()
      } catch (error: unknown) {
        showV4CreativeError(error, fallbackMessage)
        return null
      } finally {
        setIsV4CreativeBusy(false)
      }
    },
    [isV4CreativeBusy, showV4CreativeError]
  )
  const applyV4Workflows = useCallback((rows: V4Workflow[]) => {
    setV4Workflows(rows)
    setV4SelectedWorkflowId((current) =>
      current && rows.some((item) => item.id === current) ? current : rows[0]?.id || ''
    )
  }, [])
  const applyV4WorkflowRunsPage = useCallback(
    (
      rows: V4WorkflowRun[],
      append: boolean,
      limit: number,
      ...
      const cursorFromPage =
        typeof page?.nextCursor === 'string'
          ...
          : typeof page?.cursor === 'string'
            ...
            : inferredCursor
      const hasMore =
        typeof page?.hasMore === 'boolean' ? page.hasMore : rows.length >= Math.min(limit, 200)
  ...
  const refreshV4Workflows = useCallback(async () => {
    const payload = await runV4CreativeTask(
      () => requestV4<{ success: boolean; workflows: V4Workflow[] }>('/creative/prompt-workflows'),
      '加载 workflow 失败',
      { guardBusy: false }
    )
    if (!payload) return
    applyV4Workflows(payload.workflows || [])
  }, [applyV4Workflows, runV4CreativeTask])
  ...
  const createV4Workflow = useCallback(async () => {
    ...
    const payload = await runV4CreativeTask(
      () =>
        requestV4<{ success: boolean; workflow: V4Workflow }>(
  ...
  }, [runV4CreativeTask, showToast, v4WorkflowDescription, v4WorkflowName])
  ...
  const runV4Workflow = useCallback(async () => {
    ...
    const payload = await runV4CreativeTask(
      () =>
        requestV4<{ success: boolean; run: V4WorkflowRun }>(
  ...
  const queryV4WorkflowRuns = useCallback(
    async (append = false) => {
      ...
      if (!payload) return
      applyV4WorkflowRunsPage(payload.runs || [], append, limit, payload.page)
    },
    [
      applyV4WorkflowRunsPage,
      runV4CreativeTask,
      showToast,
      v4SelectedWorkflowId,
      v4WorkflowRunsCursor,
      v4WorkflowRunsLimit
    ]
  )
  ...
  const createV4BatchJob = useCallback(async () => {
    ...
    if (!input) return
    const payload = await runV4CreativeTask(async () => {
      const workflowRunId =
      ...
      return requestV4<{ success: boolean; job: V4BatchJob }>('/creative/batch-jobs', {
        method: 'POST',
        body: JSON.stringify({
          workflowRunId,
  ...
  }, [parseJsonObjectInput, runV4CreativeTask, showToast, v4BatchJobPayload, v4BatchJobType])
  ...
  const queryV4BatchJob = useCallback(async () => {
    ...
    const payload = await runV4CreativeTask(
      () =>
        requestV4<{ success: boolean; job: V4BatchJob }>(
  ...
  }, [runV4CreativeTask, showToast, v4BatchJobId])
  ...
  const callV4AssetReuse = useCallback(async () => {
    ...
    const payload = await runV4CreativeTask(
      () =>
        requestV4<{ success: boolean; record: V4AssetReuseResult }>(
  ...
  const queryV4AssetReuseHistory = useCallback(async () => {
    ...
    const payload = await runV4CreativeTask(
      () =>
        requestV4<{ success: boolean; records: V4AssetReuseRecord[] }>(
