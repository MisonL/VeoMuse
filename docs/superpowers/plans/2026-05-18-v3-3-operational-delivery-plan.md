...
- Remote CI follow-up:
  ...
  - Third push confirmed the Command Rail bus remains semantically present but visually hidden in hosted Docker Delivery. The spec now treats `command-rail-flow` as a semantic Nebula surface and keeps visibility checks on the deployed surfaces that are intended to render visibly.
