...
describe('口型同步链路验证', () => {
  ...
  it('provider 未配置时应返回 not_implemented', async () => {
    ...
    const result = await LipSyncService.sync('https://video.local/a.mp4', 'https://audio.local/a.mp3');
  ...
  it('provider 可用时应返回 syncedVideoUrl', async () => {
    ...
    const result = await LipSyncService.sync('https://video.local/a.mp4', 'https://audio.local/a.mp3', 'high');
