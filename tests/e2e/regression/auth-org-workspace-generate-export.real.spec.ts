...
test('@real 可通过真实链路串通注册 -> 组织 -> 工作区 -> 生成 -> 导出', async ({ page }) => {
  ...
  await expect(page.getByTestId('select-organization')).toBeVisible({ timeout: 20000 })
  ...
  await expect(page.getByTestId('text-workspace-id')).not.toContainText('workspace: -', { timeout: 20000 })
