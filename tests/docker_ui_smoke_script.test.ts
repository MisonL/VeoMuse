...
describe('docker UI smoke 接入', () => {
  it('根脚本应提供 docker:ui-smoke 命令', () => {
    ...
    expect(pkg?.scripts?.['docker:ui-smoke']).toBe(
      'env -u NO_COLOR PLAYWRIGHT_BASE_URL=http://127.0.0.1:18081 PLAYWRIGHT_API_BASE_URL=http://127.0.0.1:18081 bunx playwright test -c playwright.docker.config.ts --project=docker-smoke-chromium --workers=1 tests/e2e/docker/auth-org-workspace-ui.spec.ts tests/e2e/docker/all-ui-surfaces.spec.ts tests/e2e/docker/experiment-bus-overlay.spec.ts tests/e2e/docker/nebula-core-surfaces.spec.ts'
    )
    expect(pkg?.scripts?.['docker:ui-smoke']).toContain('auth-org-workspace-ui.spec.ts')
    expect(pkg?.scripts?.['docker:ui-smoke']).toContain('all-ui-surfaces.spec.ts')
    expect(pkg?.scripts?.['docker:ui-smoke']).toContain('experiment-bus-overlay.spec.ts')
    expect(pkg?.scripts?.['docker:ui-smoke']).toContain('nebula-core-surfaces.spec.ts')
  })
