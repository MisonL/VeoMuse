...
describe('多租户渠道与权限回归', () => {
  it('应隔离组织数据并支持工作区渠道覆写命中', async () => {
    ...
    const saveWorkspaceOverride = await callApi(`/api/workspaces/${workspaceId}/channels/veo-3.1`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${tokenA}`,
        'Content-Type': 'application/json',
    ...
    const orgConfig = listOrgChannels.data.configs.find((item: any) => item.providerId === 'veo-3.1')
    ...
    const workspaceConfig = listWorkspaceChannels.data.configs.find((item: any) => item.providerId === 'veo-3.1')
    expect(workspaceConfig.baseUrl).toBe('http://127.0.0.1:18081')
    ...
    expect(String(generateWithOrgConfig.data.error || '')).toContain('URL is invalid')
    ...
    expect(String(generateWithWorkspaceOverride.data.error || '')).not.toContain('URL is invalid')
    ...
    expect(String(generateOnOrgB.data.error || '')).toContain('URL is invalid')
  })
  ...
  it('应支持 OpenAI 兼容渠道并命中工作区覆写', async () => {
    ...
    const createMockServer = (label: string, calls: Array<{ path: string; auth: string; body: any }>) => Bun.serve({
      hostname: '127.0.0.1',
      port: 0,
      async fetch(request) {
    ...
    try {
      ...
      const saveOrgChannel = await callApi(`/api/organizations/${orgId}/channels/openai-compatible`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      const saveWorkspaceChannel = await callApi(`/api/workspaces/${workspaceId}/channels/openai-compatible`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      expect(listWorkspaceChannels.response.status).toBe(200)
      const workspaceConfig = listWorkspaceChannels.data.configs.find((item: any) => item.providerId === 'openai-compatible')
      expect(workspaceConfig.workspaceId).toBe(workspaceId)
