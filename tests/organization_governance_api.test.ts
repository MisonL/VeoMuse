...
describe('组织治理与配额 API', () => {
  it('应支持组织配额读写权限控制（member 可读、admin 可写）', async () => {
    ...
    const addMemberResp = await app.handle(new Request(`http://localhost/api/organizations/${owner.organizationId}/members`, {
      method: 'POST',
    ...
    const memberReadResp = await app.handle(new Request(`http://localhost/api/organizations/${owner.organizationId}/quota`, {
    ...
    const memberWriteResp = await app.handle(new Request(`http://localhost/api/organizations/${owner.organizationId}/quota`, {
      method: 'PUT',
      headers: createAuthHeaders(member.accessToken, {
        organizationId: owner.organizationId,
        contentTypeJson: true
    ...
    const ownerWriteResp = await app.handle(new Request(`http://localhost/api/organizations/${owner.organizationId}/quota`, {
      method: 'PUT',
      headers: createAuthHeaders(owner.accessToken, {
        organizationId: owner.organizationId,
        contentTypeJson: true
  ...
  it('应在请求配额耗尽后阻断视频生成', async () => {
    ...
    const setQuotaResp = await app.handle(new Request(`http://localhost/api/organizations/${session.organizationId}/quota`, {
      method: 'PUT',
    ...
    const firstGenerateResp = await app.handle(new Request('http://localhost/api/video/generate', {
      method: 'POST',
      headers: createAuthHeaders(session.accessToken, {
        organizationId: session.organizationId,
        contentTypeJson: true
    ...
    const secondGenerateResp = await app.handle(new Request('http://localhost/api/video/generate', {
      method: 'POST',
      headers: createAuthHeaders(session.accessToken, {
        organizationId: session.organizationId,
        contentTypeJson: true
  ...
  it('应在存储配额耗尽后阻断本地导入', async () => {
    ...
    const firstImportResp = await app.handle(new Request('http://localhost/api/storage/local-import', {
      method: 'POST',
      headers: createAuthHeaders(session.accessToken, {
        organizationId: session.organizationId,
        contentTypeJson: true
    ...
    const secondImportResp = await app.handle(new Request('http://localhost/api/storage/local-import', {
      method: 'POST',
      headers: createAuthHeaders(session.accessToken, {
        organizationId: session.organizationId,
        contentTypeJson: true
  ...
  it('应支持审计导出并确保组织隔离', async () => {
    ...
    const createOrgAChannelResp = await app.handle(new Request(`http://localhost/api/organizations/${ownerA.organizationId}/channels/openai-compatible`, {
      method: 'PUT',
    ...
    const workspaceResp = await app.handle(new Request('http://localhost/api/workspaces', {
      method: 'POST',
      headers: createAuthHeaders(ownerA.accessToken, {
        organizationId: ownerA.organizationId,
        contentTypeJson: true
    ...
    const snapshotResp = await app.handle(new Request(`http://localhost/api/projects/${projectId}/snapshots`, {
      method: 'POST',
      headers: createAuthHeaders(ownerA.accessToken, {
        organizationId: ownerA.organizationId,
        contentTypeJson: true
    ...
    const createOrgBChannelResp = await app.handle(new Request(`http://localhost/api/organizations/${ownerB.organizationId}/channels/openai-compatible`, {
      method: 'PUT',
    ...
    const exportJsonResp = await app.handle(new Request(`http://localhost/api/organizations/${ownerA.organizationId}/audits/export?format=json&scope=all&limit=200`, {
      headers: createAuthHeaders(ownerA.accessToken, {
        organizationId: ownerA.organizationId
      })
    }))
    ...
    expect(exportJsonData.records.every((item: any) => item.organizationId === ownerA.organizationId)).toBe(true)
    ...
    const exportCsvResp = await app.handle(new Request(`http://localhost/api/organizations/${ownerA.organizationId}/audits/export?format=csv&scope=all&limit=200`, {
      headers: createAuthHeaders(ownerA.accessToken, {
        organizationId: ownerA.organizationId
      })
    }))
